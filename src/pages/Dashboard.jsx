...
export default function Dashboard() {
  ...
  const updateMetric = async (field, value) => {
    ...
    try {
      const existing = await sb(`dashboard_metrics?profile_id=eq.${getProfileId()}&metric_date=eq.${today}`)
      if (existing?.length) {
        ...
        await sb('dashboard_metrics', { method: 'POST', body: JSON.stringify({ profile_id: getProfileId(), metric_date: today, [field]: parseInt(value) }) })
      }
      fetchMetrics()
    } catch {}
  }
