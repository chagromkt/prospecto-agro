...
export default function Dashboard() {
  ...
  const fetchMetrics = async () => {
    setLoading(true)
    try {
      // Busca métricas reais da view dashboard_summary
      const [summary, manual] = await Promise.all([
        sb(`dashboard_summary?profile_id=eq.${getProfileId()}&select=*`),
        sb(`dashboard_metrics?profile_id=eq.${getProfileId()}&order=metric_date.desc&limit=1`).catch(() => [])
      ])
      ...
      if (s) {
        setMetrics({
          ...
          connections_this_week: s.connections_this_week,
          connections_this_month: s.connections_this_month,
          conversations_this_month: s.conversations_this_month,
          conversations_with_reply: s.conversations_with_reply,
          total_conversations: s.total_conversations,
          comments_this_month: s.comments_this_month,
          likes_this_month: s.likes_this_month,
          visits_this_month: s.visits_this_month,
          messages_this_month: s.messages_this_month,
          total_connections: s.total_connections,
          // Dados manuais (editáveis)
          meetings_this_month: man.meetings_booked ?? 0,
          proposals_this_month: man.proposals_sent ?? 0,
          sales_this_month: man.sales_closed ?? 0,
          revenue_this_month: man.revenue_amount ?? 0,
        })
      } else {
        // View indisponível — estado vazio explícito (não zeros mascarados)
        setMetrics(null)
      }
    } catch(e) {
      console.error('fetchMetrics error:', e)
      setMetrics(null) // erro explícito, não zeros
    }
    setLoading(false)
  }
  ...
  return (
    <div style={{ flex: 1, padding: '28px 32px', overflowY: 'auto', background: '#ffffff' }}>
      ...
      {loading ? (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: 200, color: '#9a9ab0' }}>Carregando...</div>
      ) : metrics === null ? (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: 200, gap: 10 }}>
          <div style={{ fontSize: 32 }}>⚠️</div>
          <div style={{ fontSize: 14, color: '#9a9ab0' }}>Erro ao carregar métricas</div>
          <button onClick={fetchMetrics} style={{ background: '#f0faf4', border: '1px solid #b8e8c8', borderRadius: 8, color: '#1e6b3a', padding: '6px 14px', fontSize: 12, cursor: 'pointer' }}>Tentar novamente</button>
        </div>
      ) : (<>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12, marginBottom: 28 }}>
          ...
          <MetricCard label="Campanhas Ativas" value={m.active_campaigns ?? 0} color="#7c3aed" />
        </div>
