...
export default function Conteudo() {
  ...
  const statusColor = s => s === 'published' ? '#4ade80' : s === 'scheduled' ? '#f59e0b' : '#3d5a3d'
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      <div style={{ padding: '20px 32px', borderBottom: '1px solid #162018', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <h1 style={{ fontSize: 20, color: '#c8b76a', fontWeight: 700 }}>Conteúdo LinkedIn</h1>
          <p style={{ fontSize: 12, color: '#3d5a3d', marginTop: 4 }}>Gere e agende posts com IA</p>
        </div>
        <button onClick={() => setShowNew(true)} style={{ background: 'linear-gradient(135deg,#1e5c2c,#2d8a40)', border: 'none', borderRadius: 8, color: '#e8e4d9', padding: '10px 20px', fontSize: 13 }}>+ Novo Post</button>
      </div>

      <div style={{ flex: 1, overflowY: 'auto', padding: '24px 32px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 16 }}>
          {posts.map(post => (
            <div key={post.id} style={{ background: '#0d1a0f', border: '1px solid #162018', borderRadius: 12, padding: 20, display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                {post.scheduled_for && <span style={{ fontSize: 10, color: '#3d5a3d' }}>{new Date(post.scheduled_for).toLocaleDateString('pt-BR', { day:'2-digit', month:'short', hour:'2-digit', minute:'2-digit' })}</span>}
              </div>
              <p style={{ fontSize: 13, color: '#a89a6a', lineHeight: 1.6, flex: 1, display: '-webkit-box', WebkitLineClamp: 6, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                {post.final_text || post.generated_text || '(rascunho vazio)'}
              </p>
              {post.status === 'published' && (
                <div style={{ display: 'flex', gap: 16, paddingTop: 8, borderTop: '1px solid #162018' }}>
                  {[['❤️', post.reactions || 0], ['💬', post.comments || 0], ['↩', post.reposts || 0]].map(([icon, v], i) => (
                    <span key={i} style={{ fontSize: 12, color: '#3d6b4a' }}>{icon} {v}</span>
                  ))}
                </div>
              )}
            </div>
      ...
      {showNew && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <div style={{ background: '#0d1a0f', border: '1px solid #1e3322', borderRadius: 14, padding: 28, width: 680, maxHeight: '90vh', overflowY: 'auto' }}>
            <h2 style={{ fontSize: 16, color: '#c8b76a', marginBottom: 20, fontWeight: 700 }}>Criar Post com IA</h2>

            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
              <div>
                <div style={{ fontSize: 11, color: '#3d5a3d', marginBottom: 6 }}>TEMA / PROMPT</div>
              ...
              <div>
                <div style={{ fontSize: 11, color: '#3d5a3d', marginBottom: 6 }}>CONTEXTO / BASE DE CONHECIMENTO</div>
            ...
            <div style={{ display: 'flex', gap: 12, marginBottom: 16 }}>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 11, color: '#3d5a3d', marginBottom: 6 }}>TOM DE VOZ</div>
              ...
              <button onClick={generate} disabled={generating} style={{ alignSelf: 'flex-end', background: 'linear-gradient(135deg,#1e5c2c,#2d8a40)', border: 'none', borderRadius: 8, color: '#e8e4d9', padding: '10px 20px', fontSize: 13, minWidth: 140 }}>
            ...
            {form.generated_text && (
              <div style={{ marginBottom: 16 }}>
                <div style={{ fontSize: 11, color: '#3d5a3d', marginBottom: 6 }}>TEXTO GERADO — EDITE SE NECESSÁRIO</div>
                ...
                <div style={{ fontSize: 11, color: '#2d5a3d', marginTop: 4 }}>{(form.final_text || form.generated_text).length} / 3000 caracteres</div>
              </div>
            )}

            <div style={{ marginBottom: 20 }}>
              <div style={{ fontSize: 11, color: '#3d5a3d', marginBottom: 6 }}>AGENDAR PUBLICAÇÃO (opcional)</div>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button onClick={schedule} style={{ flex: 2, background: 'linear-gradient(135deg,#1e5c2c,#2d8a40)', border: 'none', borderRadius: 8, color: '#e8e4d9', padding: 10, fontSize: 13 }}>
