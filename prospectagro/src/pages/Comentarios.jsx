  ...
  return (
    <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        <div style={{ padding: '20px 32px', borderBottom: '1px solid #162018', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <h1 style={{ fontSize: 20, color: '#c8b76a', fontWeight: 700 }}>Agente de Comentários</h1>
            <p style={{ fontSize: 12, color: '#3d5a3d', marginTop: 4 }}>Comenta automaticamente nos posts dos seus leads</p>
          </div>
          <button onClick={() => setShowNew(true)} style={{ background: 'linear-gradient(135deg,#1e5c2c,#2d8a40)', border: 'none', borderRadius: 8, color: '#e8e4d9', padding: '10px 20px', fontSize: 13 }}>+ Nova Campanha</button>
        </div>

        <div style={{ flex: 1, overflowY: 'auto', padding: '24px 32px', display: 'flex', gap: 24 }}>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 11, color: '#3d5a3d', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 12 }}>Campanhas Ativas</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {camps.map(c => (
                <div key={c.id} style={{ background: '#0d1a0f', border: '1px solid #162018', borderRadius: 12, padding: 20 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
                    <div>
                      <div style={{ fontSize: 14, color: '#c8b76a', fontWeight: 700, marginBottom: 4 }}>{c.name}</div>
                      <div style={{ fontSize: 12, color: '#5a7a5a' }}>{c.objective}</div>
                    </div>
                    <button onClick={() => toggle(c)} style={{ background: c.is_active ? '#0f2a18' : '#1a0f0f', border: c.is_active ? '1px solid #2d6a3f' : '1px solid #3a1a1a', borderRadius: 20, padding: '6px 14px', fontSize: 12, color: c.is_active ? '#4ade80' : '#f87171' }}>
                  ...
                  <div style={{ display: 'flex', gap: 20 }}>
                    {[['Tom', c.tone_of_voice],['Por dia', c.max_comments_per_day],['Por pessoa', c.max_comments_per_person],['Total', c.total_comments || 0]].map(([l,v]) => (
                      <div key={l}>
                        <div style={{ fontSize: 10, color: '#2d4a2d', textTransform: 'uppercase' }}>{l}</div>
                        <div style={{ fontSize: 13, color: '#7a9a7a', marginTop: 2 }}>{v}</div>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          <div style={{ width: 340 }}>
            <div style={{ fontSize: 11, color: '#3d5a3d', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 12 }}>Últimos Comentários</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {logs.map(log => (
                <div key={log.id} style={{ background: '#0d1a0f', border: '1px solid #162018', borderRadius: 10, padding: 14 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                    <span style={{ fontSize: 12, color: '#c8b76a' }}>{log.leads?.full_name}</span>
                    <span style={{ fontSize: 10, color: '#2d5a3d' }}>{new Date(log.sent_at).toLocaleTimeString('pt-BR', { hour:'2-digit', minute:'2-digit' })}</span>
                  </div>
                  <p style={{ fontSize: 12, color: '#7a9a7a', lineHeight: 1.5, display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{log.comment_text}</p>
                  <div style={{ fontSize: 10, color: '#2d4a2d', marginTop: 6 }}>{log.leads?.current_company}</div>
                </div>
              ))}
            </div>
          </div>
      ...
      {showNew && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <div style={{ background: '#0d1a0f', border: '1px solid #1e3322', borderRadius: 14, padding: 28, width: 500 }}>
            <h2 style={{ fontSize: 16, color: '#c8b76a', marginBottom: 20, fontWeight: 700 }}>Nova Campanha de Comentários</h2>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
              <div>
                <div style={{ fontSize: 11, color: '#3d5a3d', marginBottom: 6, textTransform: 'uppercase' }}>Por dia (máx)</div>
              ...
              <div>
                <div style={{ fontSize: 11, color: '#3d5a3d', marginBottom: 6, textTransform: 'uppercase' }}>Por pessoa (máx)</div>
            ...
            <div style={{ marginBottom: 20 }}>
              <div style={{ fontSize: 11, color: '#3d5a3d', marginBottom: 6, textTransform: 'uppercase' }}>Tom de voz</div>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button onClick={save} style={{ flex: 2, background: 'linear-gradient(135deg,#1e5c2c,#2d8a40)', border: 'none', borderRadius: 8, color: '#e8e4d9', padding: 10, fontSize: 13 }}>Criar Campanha</button>
            </div>
          </div>
        </div>
      )}
